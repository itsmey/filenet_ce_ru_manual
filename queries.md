...
Используется в клаузе WHERE, в условиях, где сравниваются значения свойств и константы. Перечислим типы и операторы, применимые к свойствам и канстантам этих типов:

* Binary: свойства этого типа недопустимы в запросах
* Boolean: '=' | '<>' 
* DateTime: '=' | '<>' | '<' | '>' | '<=' | '>=' 
* Float: '=' | '<>' | '<' | '>' | '<=' | '>=' 
* ID: '=' | '<>' | '<' | '>' | '<=' | '>=' 
...
Для всех типов (кроме Binary) CE SQL предоставляет дополнительный оператор сравнения IS [NOT] NULL, позволяющий проверять значения на равенство или неравенство null. 

Если данный оператор применяется к свойству мощности LIST, это означает проверку на пустоту списка.

```sql
    SELECT Id FROM Document WHERE DocumentTitle IS NULL
```
...
* NOW() возвращает константу типа DateTime, соответствующее текущему времени на сервере в стандарте UTC. Функция очень удобна, когда надо, например, найти документы с истёкшим сроком действия: 

```sql
    SELECT DocumentTitle, ExpiryDate FROM MyUrgentDocument WHERE ExpiryDate < NOW()
```

* TIMESPAN(<количество>, <единицы измерения>) - формирует "константу сдвига". Константы сдвига можно суммировать с датами или отнимать от них в условиях сравнения дат. Например, найти все документы, срок действия которых истёк за последние 7 дней: 

```sql
    SELECT DocumentTitle, ExpiryDate FROM MyUrgentDocument WHERE ExpiryDate < NOW() - TIMESPAN(7, 'days')
```
