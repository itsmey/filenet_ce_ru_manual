...
Также следует иметь в виду, что не все свойства можно указывать в клаузах SELECT и WHERE. Это относится, например, к свойству ContentELements (элементы содержимого), т.к. оно может существенно увеличить время выполнения запроса.
