...
Также следует учесть, что окончательные права на объект определяются не только значением, возвращаемым методом getAccessAllowed(), но и маркировками (если они есть). См. [Наборы маркировок](marking_sets.md).
