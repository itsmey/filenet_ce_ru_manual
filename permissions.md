...
**значение** | **действие**
------------ | -------------
ADD_MARKING|Назначение маркировки объекту. См. [Наборы маркировок](marking_sets.md)
...
REMOVE_MARKING|Удаление маркировки. См. [Наборы маркировок](marking_sets.md)
...
USE_MARKING|См. [Наборы маркировок](marking_sets.md)
...
## Определение прав текущего пользователя на объект

Существует метод `IndependentlyPersistableObject.getAccessAllowed()`, которой возвращает битовую маску прав, который имеет текущий пользователь на объект, вызывающий метод. Это полезно тогда, когда нужно сделать какие-то действия в зависимости от того, обладает ли пользователь некими правами на объект или нет.

Также следует учесть, что права на объект, возвращаемые методом getAccessAllowed(), определяются не только набором разрешений (св-во Permissions), но и маркировками (если они есть). См. [Наборы маркировок](marking_sets.md).
...
* RIGHT_INHERIT_ONLY - одиночное право, применяемое только к дочерним объектам
