...
В CE список выбора состоит, упрощенно говоря, из коллекций пар "значение для отображения" (типа String) - "внутреннее значение" (типа Integer или String). Такая пара называется "элемнтом варианта".
...
`HasHierarchy`|Boolean|Иерархический ли список. Список иерархический, если хотя бы один из элементов варианта в коллекции ChoiceValues является группой (имеет непустую коллекцию ChoiceValues). Свойство только для чтения
